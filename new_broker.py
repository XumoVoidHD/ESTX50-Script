...
class IBTWSAPI:
    ...
    async def get_option_chain(self, symbol: str, exp_list: list) -> dict:
        ...
        for i in exp_list:
            ...
            for x in options:
                # print(x)
                contract = Option(symbol, i, x.strike, x.right, "SMART", currency=credentials.currency)
                # print(contract)
    ...
    async def get_latest_premium_price(self, symbol, expiry, strike, right, exchange="EUREX", print_data=False):
        ...
        option_contract = Option(
            ...
            right=right,
            exchange=exchange,
            currency=credentials.currency,  # Add currency to disambiguate
            multiplier="10",  # Ensure the multiplier matches
